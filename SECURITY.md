...
- 所有改变状态的 HTTP 请求要求 CSRF token 和同源 Origin。
...
中风险：启动、停止、重启、修改允许的 properties、白名单、OP、上传/停用模组、调整定时备份。需要明确确认和影响摘要。
...
- 模型声称成功不改变状态；只有后端工具的结构化结果可以完成操作。结果会区分“命令已发送”和“状态已验证”，不会统一伪装成已验证。
