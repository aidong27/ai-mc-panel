...
- 服务端 `OperationReview` 与 `ReviewItem`，统一展示玩家、属性、模组、备份、服务和手动命令的精确目标。
- AI 工具暴露等级：只读、AI 可提议和仅手动；运行时 schema 与 `AGENT_TOOLS.md` 由严格测试保持一致。
- 浏览器验收覆盖 AI 提案到后端确认、取消/执行，以及手动控制台逐字确认。

### Changed

- 确认记录冻结规范化参数与完整 review，并校验整体哈希及 action + params 哈希。
- 通用控制台入口由 root 能力开关控制，生产和示例配置默认关闭；关闭时日志读取仍可使用。
- 确认页增加后端生成的具体目标和预期核对方式，移动端保持单列可读。

### Security

- `send_console_command` 改为仅手动工具，不再出现在模型 schema；供应商伪造该 tool call 也不会产生提案。
- systemd 适配器从隔离区读取模组 review 元数据时验证固定 ID、所有者、模式、大小、JSON 重复键和 `O_NOFOLLOW` 文件对象。
- 文档不再把尚未统一分级的操作结果笼统声明为“已验证”。
